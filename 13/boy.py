...
class Boy:
    ...
    def update_x(self, BrickVelocity, Brick_Left, Brick_Right):
        if self.CollideCheck_Brick == False:
            return

        self.brick_rigjht = Brick_Right
        self.brick_left = Brick_Left
        self.x += BrickVelocity
        if self.CollideCheck_Brick == True:
            if self.x >= self.brick_rigjht :
                self.x = self.brick_rigjht
            elif self.x <= self.brick_left :
                self.x = self.brick_left

    def update(self):
        if self.y > 90:
            self.y -= 500 * game_framework.frame_time

        # if self.CollideCheck_Brick == True:
        #     if self.x > self.brick_rigjht - 90:
        #         self.x = self.brick_rigjht - 90
        #     elif self.x < self.brick_left + 90:
        #         self.x = self.brick_left + 90
